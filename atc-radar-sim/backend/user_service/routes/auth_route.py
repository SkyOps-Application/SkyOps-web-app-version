...
from utils.errors import UnauthorizedException, NotFoundException, ForbiddenException
...
def reset_password():
    ...
    if (user.first_name.strip().lower() != schema.first_name.strip().lower() or 
        ...
        raise ForbiddenException("Identity verification failed. Name does not match records.")
